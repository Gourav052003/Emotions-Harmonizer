...
from keras.layers import Input,Dense,LeakyReLU,Add,BatchNormalization,Concatenate,Reshape,Flatten
...
class ModelArchitecture:
    ...
    def Encoder_Decoder_model(self):
        ...
        for unit_no in range(self.config.encoder_timestamp):
            
            if unit_no == 0:
                i = Input((self.config.sample_rate),name = f"encoder_Input_1_unit_{unit_no}")
                r = self.relation_neural_network(i = i,ou = [i],inter_i = None,unit_no = unit_no,architecture_type="encoder",sample_rate = self.config.sample_rate,timestamp = 2)
                h = self.hidden_layers(r,unit_no,architecture_type = "encoder")
                d = Dense(self.config.sample_rate)(h)
                ou = LeakyReLU(alpha = 1)(d)

            else:
                i = Input((self.config.sample_rate),name = f"encoder_Input_1_unit_{unit_no}")
                r = self.relation_neural_network(i = i,ou = encoder_output_units.copy(),inter_i = None,unit_no = unit_no,architecture_type="encoder",sample_rate = self.config.sample_rate,timestamp = unit_no+1)
                h = self.hidden_layers(r,unit_no,architecture_type = "encoder")
                d = Dense(self.config.sample_rate)(h)
                ou = LeakyReLU(alpha = 1)(d)
        ...
        r = self.relation_neural_network(inter_i = encoder_output_units.copy(),unit_no = 777,architecture_type="inter",sample_rate = self.config.sample_rate,timestamp = self.config.encoder_timestamp)
        h = self.hidden_layers(r,unit_no,architecture_type = "inter")
        d = Dense(self.config.sample_rate)(h)
        inter_i = LeakyReLU(alpha = 1)(d)


        for unit_no in range(self.config.decoder_timestamp):

            if unit_no == 0:
                r = self.relation_neural_network(i = inter_i,ou = [inter_i],inter_i = None,unit_no = unit_no,architecture_type="decoder",sample_rate = self.config.sample_rate,timestamp = 2)
                h = self.hidden_layers(r,unit_no,architecture_type = "decoder")
                d = Dense(self.config.sample_rate)(h)
                ou = LeakyReLU(alpha = 1)(d)

            else:
                r = self.relation_neural_network(i = inter_i,ou = decoder_output_units.copy(),inter_i = None,unit_no = unit_no,architecture_type="decoder",sample_rate = self.config.sample_rate,timestamp = unit_no+1)
                h = self.hidden_layers(r,unit_no,architecture_type = "decoder")
                d = Dense(self.config.sample_rate)(h)
                ou = LeakyReLU(alpha = 1)(d)
        ...
        model.compile(optimizer = Adam(learning_rate=self.config.learning_rate),loss=self.config.loss)
        model.summary()
